...
Cellcoin Core version *0.15.0* is now available from:

  <https://download.cellcoin.org/cellcoin-0.15.0/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://groups.google.com/forum/#!forum/cellcoin-dev>
...
installer (on Windows) or just copy over `/Applications/Cellcoin-Qt` (on Mac)
or `cellcoind`/`cellcoin-qt` (on Linux).
...
Cellcoin Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.8+, and Windows Vista and later. Windows XP is not supported.

Cellcoin Core should also work on most other Unix-like systems but is not
...
- The `estimatesmartfee` RPC interface has been changed (See [PR 10707](https://github.com/bitcoin/bitcoin/pull/10707)):
    ...
    - If Cellcoin Core has not been running for long enough and has not seen enough blocks or transactions to produce an accurate fee estimation, an error will be returned (previously a value of -1 was used to indicate an error, which could be confused for a feerate).
...
Cellcoin Core now supports loading multiple, separate wallets (See [PR 8694](https://github.com/bitcoin/bitcoin/pull/8694), [PR 10849](https://github.com/bitcoin/bitcoin/pull/10849)). The wallets are completely separated, with individual balances, keys and received transactions.

Multi-wallet is enabled by using more than one `-wallet` argument when starting Cellcoin, either on the command line or in the Cellcoin config file.
...
Cellcoin Core 0.15.0 contains the following changes to the RPC interface and `cellcoin-cli` for multi-wallet:

* When running Cellcoin Core with a single wallet, there are **no** changes to the RPC interface or `cellcoin-cli`. All RPC calls and `cellcoin-cli` commands continue to work as before.
* When running Cellcoin Core with multi-wallet, all *node-level* RPC methods continue to work as before. HTTP RPC requests should be send to the normal `<RPC IP address>:<RPC port>/` endpoint, and `cellcoin-cli` commands should be run as before. A *node-level* RPC method is any method which does not require access to the wallet.
* When running Cellcoin Core with multi-wallet, *wallet-level* RPC methods must specify the wallet for which they're intended in every request. HTTP RPC requests should be send to the `<RPC IP address>:<RPC port>/wallet/<wallet name>/` endpoint, for example `127.0.0.1:8332/wallet/wallet1.dat/`. `cellcoin-cli` commands should be run with a `-rpcwallet` option, for example `cellcoin-cli -rpcwallet=wallet1.dat getbalance`.
...
In previous versions of Cellcoin Core, a portion of each block could be reserved for transactions based on the age and value of UTXOs they spent. This concept (Coin Age Priority) is a policy choice by miners, and there are no consensus rules around the inclusion of Coin Age Priority transactions in blocks. In practice, only a few miners continue to use Coin Age Priority for transaction selection in blocks. Cellcoin Core 0.15 removes all remaining support for Coin Age Priority (See [PR 9602](https://github.com/bitcoin/bitcoin/pull/9602)). This has the following implications:
...
- When using Cellcoin Core in multi-wallet mode, RPC requests for wallet methods must specify
